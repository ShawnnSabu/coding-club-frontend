...
export default function Login() {
  return (
    <div className="login-root">
      <style>{`
        :root{
          --bg:#0b0b0c; --panel:#141419; --panel2:#1a1b22;
          --text:#f5f5f5; --muted:#bdbdbf; --accent:#ff2b2b;
          --input:#2a2c36; --inputFocus:#353746; --ring:rgba(255,255,255,.12);
          --cardBorder:rgba(255,255,255,.18); --gradA:#7b5cff; --gradB:#ff7b6e;
        }
        .login-root{ color:var(--text); background:var(--bg); min-height:100dvh; overflow-x:hidden }
        .container{ max-width:1180px; margin:0 auto; padding:56px 24px 110px; position:relative }
        .rails{ position:absolute; inset:0; pointer-events:none }
        .rail{ position:absolute; top:40px; bottom:40px; width:2px;
        ...
        .brand{ display:flex; align-items:center; gap:10px; margin-bottom:12px; font-weight:700 }
        ...
        .title{ font-size:clamp(44px,6vw,84px); font-weight:800; letter-spacing:-1px; margin:10px 0 30px }
        .angle{ color:var(--accent) }

        .card{
          position:relative; border-radius:28px; border:1.5px solid var(--cardBorder);
          background:
...
<path d='M0,280 C70,250 140,290 220,270 S360,230 400,250' fill='none' stroke='rgba(255,255,255,0.05)' stroke-width='1'/>\
          ...
          padding:38px 34px; box-shadow:0 10px 30px rgba(0,0,0,.35), inset 0 0 0 1px #ffffff10;
        }
        .grid{ display:grid; grid-template-columns:1.1fr 1fr; gap:clamp(22px,3.5vw,40px) }
        ...
        .aboutBtn{
          margin-top:24px; display:inline-block; padding:12px 22px; color:#111; background:#fff;
          border-radius:999px; font-weight:700; border:none; cursor:pointer;
          transition:transform .12s ease, box-shadow .2s ease; box-shadow:0 6px 16px rgba(255,255,255,.15)
        }
        ...
        .extras{ margin-top:6px; display:flex; align-items:center; justify-content:space-between }
        .check{ display:inline-flex; align-items:center; gap:10px; color:var(--muted); user-select:none }
        .check input{ accent-color:#cfcfcf }
        .muted-link{ color:var(--muted); text-decoration:none }
        .muted-link:hover{ text-decoration:underline }

        .actions{ display:flex; justify-content:flex-end; margin-top:28px }
        .cta{
          height:54px; padding:0 28px; border:none; border-radius:999px; color:#111; font-weight:800; font-size:18px; cursor:pointer;
        ...
        .alt{ margin-top:14px; color:var(--muted); font-size:.95rem }
